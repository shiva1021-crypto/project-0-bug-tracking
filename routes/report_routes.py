...
from routes.bug_routes import PRIORITIES, SEVERITIES, STATUSES
...
def build_report_query(args):
    query = """
        SELECT bugs.id, bugs.title, bugs.category, bugs.status, bugs.priority, bugs.severity,
...
def reports():
    ...
    if is_csv_export:
        ...
        writer.writerow(["ID", "Bug", "Category", "Status", "Priority", "Severity", "Reporter", "Assigned", "Date"])
        for bug in bugs:
            writer.writerow(
                [
                    safe_csv_cell(value)
